...
O projeto chegou a ter um fallback para SQLite quando rodando fora do Docker.
Removi: valida o código num banco diferente do de produção, e a diferença é
real. Mesmo código, mesmos dados, resultados diferentes:

| `GET /api/v1/seguradoras/?nome=são` | resultado |
|---|---|
| SQLite | **0 registros** |
| PostgreSQL | **1 registro** |

O registro é `SÃO PAULO SEGUROS`. O filtro usa `icontains`, e o `LIKE` do SQLite
só é case-insensitive para ASCII — `são` não casa com `SÃO`. No PostgreSQL o
`icontains` vira `UPPER(...) LIKE UPPER(...)`, que trata acento corretamente.
Num catálogo com nomes em português, isso não é detalhe.

O segundo caso: SQLite ignora o `max_length` de `VARCHAR` e o PostgreSQL o
aplica. É por isso que existe o corte `[:255]` em `DadosCNPJ.de_payload` — sem
ele, uma razão social longa passaria batida em SQLite e estouraria em produção.

**Validação de CNPJ com dígito verificador**, não só contagem de dígitos —
inclusive rejeitando sequências como `11111111111111`, que passam no cálculo mas
não são CNPJs reais.

**Multi-stage build.** O estágio `builder` tem o `build-essential` e monta o
virtualenv; a imagem final recebe só o venv pronto, sem toolchain de compilação.
...
seguradoras/
├── models.py            o modelo Seguradora
├── validators.py        validação de CNPJ (dígito verificador)
├── serializers.py       validação e normalização da entrada/saída
├── filters.py           filtros de UF e nome
├── pagination.py        paginação com teto de page_size
├── views.py             camada HTTP (fina: valida, delega, responde)
├── services.py          regra do upsert em lote
├── enriquecer.py        regra do enriquecimento
├── cache_utils.py       cache da listagem com invalidação por versão
├── integrations/
│   ├── brasilapi.py     cliente HTTP + DadosCNPJ + Protocol ClienteCNPJ
│   └── exceptions.py    CNPJNaoEncontrado / ServicoIndisponivel
├── management/commands/
│   └── enriquecer_seguradoras.py    o worker
└── tests/               62 testes
```

A separação segue uma regra só: **view cuida de HTTP, service cuida de regra,
integration cuida do mundo externo.** É o que permite testar o upsert sem subir
request e testar a regra de enriquecimento sem simular HTTP.

## Comandos úteis

```bash
docker compose logs -f worker                                  # acompanhar o enriquecimento
docker compose exec web python manage.py enriquecer_seguradoras  # rodar uma vez, na mão
docker compose run --rm web pytest -v                          # testes detalhados
docker compose down -v                                         # derrubar tudo e apagar o banco
```

## O que ficou de fora

- **Autenticação/permissões.** O enunciado não pediu; a API é aberta.
- **Consultas concorrentes no worker.** Os CNPJs de uma rodada são consultados
  em sequência. Um `ThreadPoolExecutor` aceleraria lotes grandes, mas o ganho
  não se justifica no volume deste desafio e custaria bastante clareza.
- **Retry com backoff exponencial** dentro do cliente HTTP, pelo motivo
  explicado acima.
